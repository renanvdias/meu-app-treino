  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      ...
      if (history.length > 0) {
        ...
        const recentWorkouts = history.filter(session => new Date(session.date) >= sevenDaysAgo);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-24">
      ...
      <main className="p-5 max-w-md mx-auto space-y-6">
        ...
        {/* Botão com efeito de clique (active:scale) */}
        <button 
          onClick={() => setCurrentScreen('workout')}
          className="w-full bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white font-bold py-5 px-6 rounded-2xl flex justify-center items-center gap-3 transition-all shadow-md shadow-indigo-200"
        >
      ...
      {/* Menu Inferior Glassmorphism leve */}
